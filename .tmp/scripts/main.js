...
(function() {
  var chChPtn, chEl, chInputVal, chMinus, chMinusEl, chPlus, chPlusEl, chSelItem, chStepsPtn, chTStepPtn, copyPtn, copyPtnEl, delPtn, delPtnEl, getName, getVolume, mute, muteAll, muteEl, newPtn, newPtnEl, ptnNameEl, selectEl, setName, setVolume, single, singleEl, stepsEl, stepsMinus, stepsMinusEl, stepsMinusJoin, stepsMinusJoinEl, stepsPlus, stepsPlusCopy, stepsPlusCopyEl, stepsPlusEl, stepsPlusSplit, stepsPlusSplitEl, tPerStepEl, tStepMinus, tStepMinusEl, tStepPlus, tStepPlusEl, volumeEl;
  ...
  mute = function() {
    return app.getActivePattern().mute = true;
  };

  single = function() {
    muteAll();
    return app.getActivePattern().mute = false;
  };

  muteAll = function() {
    var i, ptn, _i, _len, _ref, _results;
    _ref = app.getActivePatterns();
    _results = [];
    for (i = _i = 0, _len = _ref.length; _i < _len; i = ++_i) {
      ptn = _ref[i];
      _results.push(ptn.mute = true);
    }
    return _results;
  };
  ...
  setVolume = function() {
    ...
    volumeEl.value = val;
    return app.getActivePattern().volume = val;
  };
...
(function() {
  window.initPlaySnd = function() {
    var calcPtn, cv, dev, initDev, lastBuff, recordObj, saveWaveToFile, swt;
    lastBuff = null;
    ...
    app.rec = function() {
      ...
      if (recordObj != null ? recordObj.active : void 0) {
        recordObj.stop();
        recordObj.exportWav((function(blob) {
        ...
        if (recordObj === null) {
          recordObj = dev.createRecorder();
        }
        return recordObj.start();
      }
    };
    saveWaveToFile = function(filename, link) {
      var r;
      ...
      r.onreadystatechange = function() {
        ...
        el = document.createElement("a");
        el.setAttribute("href", "data:audio/wav;charset=utf-8," + r.responseText);
        c.l(r);
        el.setAttribute("download", filename);
        return el.click();
      };
...
(function() {
  ...
  app.renderPattern = function(ptn) {
    ...
    headers = ["Tpos", "pos", "hlp", "rstT", "funct", "oct", "note", "vel", "AMspeed", "AMdepth", "PMspeed", "PMdepth"];
    COLS = headers.length;
    ROWS = ptn.steps;
    rows = ROWS;
    cols = COLS;
